...
def calcular_aporte(renda_mensal, aporte):
    ...
    irpf_com_previdencia = df.loc[df["Descrição"] == "IRPF", "Com Previdência"].values[
        0
    ]
    irpf_sem_previdencia = df.loc[df["Descrição"] == "IRPF", "Sem Previdência"].values[
        0
    ]
...
def main():
    ...
    if botao_moderado:
        ...
        irpf_com_previdencia = df.loc[
            df["Descrição"] == "IRPF", "Com Previdência"
        ].values[0]
        irpf_sem_previdencia = df.loc[
            df["Descrição"] == "IRPF", "Sem Previdência"
        ].values[0]
    ...
    if botao_agressivo:
        ...
        st.markdown("<h3>Aportes em Fundos de Previdência</h3>", unsafe_allow_html=True)
        df_prev = tabela_prev(renda_mensal, aporte, taxa_anual, dirpf)
